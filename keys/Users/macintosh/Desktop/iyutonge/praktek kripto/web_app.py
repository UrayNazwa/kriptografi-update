...
DB_PATH = os.path.join(APP_DIR, "dsa_events.db")
...
def _init_db():
    ...
    cur = conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS events (id INTEGER PRIMARY KEY AUTOINCREMENT, ts REAL NOT NULL, section TEXT NOT NULL, data TEXT NOT NULL);")
    conn.commit()
    conn.close()
...
def log_event(section: str, payload: dict):
    try:
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        cur.execute("INSERT INTO events (ts, section, data) VALUES (?, ?, ?)", (time.time(), section or "unknown", json.dumps(payload, ensure_ascii=False)))
        conn.commit()
    except Exception:
        pass
    finally:
...
def index():
    ...
    return render_template("index.html", keys_exists=keys_exists, result=None)
...
def route_gen_keys():
    ...
    return render_template("index.html", keys_exists=True,
...
def route_encdec_name():
    ...
    log_event(result.get("section"), result)
...
def route_history():
    ...
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    for row in cur.execute("SELECT id, ts, section, data FROM events ORDER BY id DESC LIMIT 100"):
        eid, ts, section, data = row
        try:
            payload = json.loads(data)
        except Exception:
            payload = {"raw": data}
        events.append({"id": eid, "ts": ts, "section": section, "payload": payload})
    conn.close()
